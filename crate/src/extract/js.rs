...
//!
//! The extension calls `String.prototype.trim` and matches `\s` in
//! several extractors. Neither means what the Rust equivalent means:
...
//!
//! In this crate it is reachable twice over. A format name arriving as
//! `"\u{feff}css"` — three invisible bytes a Windows editor adds without
//! being asked — resolved to `css` on the extension and fell through to
//! the raw scan here, and the raw scan is where the two prose rules
//! live: a short hex needs an `a`-`f` and a named colour must be the
//! whole value. So the same call to the same shared tool answered that
//! `#250` is a colour on one server and not on the other.
//!
