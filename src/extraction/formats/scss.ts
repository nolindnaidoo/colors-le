...
/**
 * Extract colors from SCSS/Sass files
 */
					...
					if (isValidColorValue(colorValue)) {
						...
						colors.push(
							Object.freeze({
								...
								context: `SCSS ${getContextType(line)}`,
							}),
						);
					}
				}
...
function isValidColorValue(value: string): boolean {
	...
	// Skip SCSS variables and functions that aren't colors
...
/**
 * Get the context type from a line of SCSS
 */
function getContextType(line: string): string {
	if (line.includes('$')) return 'variable';
	if (line.includes('@mixin') || line.includes('@include')) return 'mixin';
