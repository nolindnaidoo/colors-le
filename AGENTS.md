...
- Some files sit above the ~300-line smell. Three do so **on purpose**:
  `extraction/heuristics.ts` is the single home for the colour patterns —
  splitting it is what produced v1's seven divergent copies, and the invariant
  above forbids it; `analysis/colorAnalysis.ts` and
  `conversion/colorConverter.ts` are each one cohesive pass, and dividing them
  would add indirection without separating concerns.

  Three more are **accepted, not justified**: `commands/analyze.ts`,
  `commands/filter.ts` and `commands/convert.ts`. Each mixes a prompt sequence
  with the work it drives, which is the seam `filterPrompts.ts` and
  `validatePrompts.ts` were already split along — these are the ones that have
  not been. Splitting them is open work, not a decision.

  **No line counts are quoted here.** They were, and all three had drifted
  from the files they described while the list still said "four files" over
  six. A stale exception list is worse than the rule it qualifies; count them
  when you need them.
