...
const CASES: readonly Case[] = [
	{
		...
		build: () =>
			Array.from({ length: 30_000 }, (_, i) =>
				`.c${i} { color: #${(i % 4096).toString(16).padStart(3, '0')}; background: rgb(${i % 256}, ${(i * 3) % 256}, ${(i * 7) % 256}); }`,
			).join('\n'),
	},
	{
		...
		build: () =>
			Array.from({ length: 30_000 }, (_, i) =>
				`$brand-${i}: hsl(${i % 360}, 50%, 50%); $alt-${i}: rgba(${i % 256}, 10, 20, 0.5);`,
			).join('\n'),
	},
	{
		label: 'HTML markup',
		arg: 'html',
		build: () =>
			`<html><body>\n${Array.from({ length: 25_000 }, (_, i) => `<div style="color:#${(i % 4096).toString(16).padStart(3, '0')};border:1px solid navy">x</div>`).join('\n')}\n</body></html>`,
	},
];

async function run(content: string, c: Case): Promise<number> {
	return (await extractColors(content, c.arg)).colors.length;
}
