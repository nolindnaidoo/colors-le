...
send({
	...
	params: {
		name: 'extract_colors',
		arguments: {
			content: 'a { color: #ff0000; border: 1px solid rgb(0, 128, 255); }',
			format: 'css',
		},
	},
});
...
setTimeout(() => {
	...
	if (init?.serverInfo?.name !== 'colors-le') {
	...
	if (call?.isError !== false) {
		fail('extract_colors reported an error', JSON.stringify(call));
	}
	const found =
		call?.structuredContent?.data?.colors?.map((c) => c.value) ?? [];
	// Asserting the actual values, not just "it responded" — a server that
	// answers with an empty list would otherwise pass.
	for (const expected of ['#ff0000', 'rgb(0, 128, 255)']) {
		if (!found.includes(expected)) {
			fail(`extract_colors did not find ${expected}`, `got: ${found.join(', ')}`);
		}
	}
