//! The Zed side of Colors-LE.
//!
//! This crate holds no extraction logic. It resolves `colors-le-mcp` from npm and
//! hands Zed a command that launches it over stdio — the same server the VSIX
//! bundles and the same one `npx colors-le-mcp` runs. Keeping it a launcher is
...
const PACKAGE_NAME: &str = "colors-le-mcp";
const SERVER_PATH: &str = "node_modules/colors-le-mcp/server.js";

struct ColorsLEExtension;
...
impl zed::Extension for ColorsLEExtension {
    fn new() -> Self {
        Self
    }
...
zed::register_extension!(ColorsLEExtension);
